...
public class badRobotController : MonoBehaviour
{
    ...
    void Update()
    {
        player_dist = Vector3.Distance(transform.position, target.position);
        
        if (timer <= 0f)
        {
            if (player_dist < seek_dist) //start pursuit if they are close enough
            {
                if (convo == false)
                {
                    //traverse to player
